...
- Fix (minimal, additive): a selection guard set in
  `HideInputFormForSelection`, followed by a 750 ms cooldown after restore.
  During the guard, the dialog temporarily removes its `AcceptButton` and
  `CancelButton`, swallows `Enter`/`Escape`/`Space` through both key-processing
  paths, and rejects `OnRunClicked`. This severs the leaked-key dismissal
...
- Code change: `src/DynLock.Addin/UI/InputForm.cs` (selection lifecycle guard,
  delayed button restore, key suppression, and run-handler guard).
